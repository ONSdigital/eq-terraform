...
credstash --table $table put -k $key "EQ_USER_AUTHENTICATION_SR_PRIVATE_KEY" "@jwt-test-keys/sdc-user-authentication-encryption-sr-private-key.pem" -a
credstash --table $table put -k $key "EQ_USER_AUTHENTICATION_SR_PRIVATE_KEY_PASSWORD" "@jwt-test-keys/sdc-user-authentication-encryption-sr-private-key-password.txt" -a
...
credstash --table $table put -k $key "EQ_SUBMISSION_SR_PRIVATE_SIGNING_KEY_PASSWORD" "@jwt-test-keys/sdc-submission-signing-sr-private-key-password.txt" -a
...
credstash --table $table put -k $key "EQ_USER_AUTHENTICATION_RRM_PRIVATE_KEY_PASSWORD" "@jwt-test-keys/sdc-user-authentication-signing-rrm-private-key-password.txt" -a


credstash --table $table put -k $key "EQ_SERVER_SIDE_STORAGE_USER_ID_SALT" "@jwt-test-keys/sdc-storage-user-id-salt.txt"
credstash --table $table put -k $key "EQ_SERVER_SIDE_STORAGE_USER_IK_SALT" "@jwt-test-keys/sdc-storage-user-ik-salt.txt"
credstash --table $table put -k $key "EQ_SERVER_SIDE_STORAGE_ENCRYPTION_USER_PEPPER" "@jwt-test-keys/sdc-storage-encryption-user-pepper.txt"
